...
% cc1=[6,4,3];
% cc2=[5,1,2];%%for Yacht Hydrodynamics data
...
% [s_COEFF1,s_latent1]= test_PCA(tr); %  
% val=diag(s_latent1);   %
...
%%%%%%%%%%%end the test
%%%%%%%%%VI acquired by original RF
%  model=regRF_train(tr,trc, num_tree,num_envfeat);
%  aa=model.importance;
%  [value,order]=sort(aa,'descend');
%  cc1=order(1:3);
%  cc2=order(4:end);
%%%%%%%%%%%%%%
for runtime=1:10
...
%%%%%%%%%%%%%end BP
jieguojilu(runtime,:)=[RMSEtr,MAEtr,R2tr,RMSEts,MAEts,R2ts,plstrrmse,plstrmae,plstrr2,plstermse,plstemae,plster2,bptrrmse,bptrmae,bptrr2,bptermse,bptemae,bpter2];
 end
